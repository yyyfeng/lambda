...
public class SensitiveConverter extends MessageConverter {
   ...
   //银行卡号正则匹配
    ...
    private static String filterIdcard(String num){
        ...
        while(matcher.find()){
            //根据*加密
            matcher.appendReplacement(sb, baseSensitiveDefault(matcher.group(), 4, 4));
            //根据要求标准加密
    ...
    private static String filterMobile(String num){
        ...
        while(matcher.find()){
            //根据*加密
            //matcher.appendReplacement(sb, baseSensitiveDefault(matcher.group(), 3, 4)) ;
            //根据要求标准加密
     ...
     * @return 脱敏后的字符串
     */
    private static String baseSensitiveDefault(String str, int startLength, int endLength) {
    ...
    private static String baseSensitive(String str, int startLength, int endLength) {
        ...
        for(int i=0;i<replacement.length();i++) {
            ...
            if(replacement.charAt(i)>='0' && replacement.charAt(i)<='9') {
                ch = KEY_1.charAt((int)(replacement.charAt(i) - '0'));
            }else {
     ...
     * @param endLength
     */
     public   String decrypt(String str, int startLength, int endLength) {
         if (StringUtils.isBlank(str)) {
             return "";
         }
         String replacement = str.substring(startLength,str.length()-endLength);
         StringBuffer sb = new StringBuffer();
         for(int i=0;i<replacement.length();i++) {
             int index = KEY_1.indexOf(replacement.charAt(i));
             if(index != -1) {
                 sb.append(index);
             }else {
